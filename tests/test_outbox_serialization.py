...
def test_outbox_to_dict_no_copy():
    """Verify that copy=False returns same references."""
    entry = OutboxEntry.create(
        agent="test-agent",
        tick=42,
        kind="test",
    ...
    actual = entry.to_dict(copy=False)

    assert actual["payload"] is entry.payload
    assert actual["tags"] is entry.tags
    assert actual["recipients"] is entry.recipients
    assert actual["meta"] is entry.meta
