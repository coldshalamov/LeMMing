...
from lemming.tools import FileWriteTool, ShellTool
...
def test_shell_output_limit(tmp_path):
    ...
    result = file_tool.execute(agent_name=agent_name, base_path=base_path, path="large.txt", content=large_content)
    ...
    result = shell_tool.execute(agent_name=agent_name, base_path=base_path, command=command)
