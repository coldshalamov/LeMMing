...
def test_agent_creation_auth_configured(client: TestClient, tmp_path):
    ...
    with (
        ...
        (source_dir / "resume.json").write_text(
            '{"name": "source", "title": "Src", "short_description": "Src", "model": '
            '{"key": "gpt"}, "permissions": {"read_outboxes": [], "tools": []}, '
        ...
        resp = client.post(
            "/api/agents/clone",
            json={"source_agent": "source", "target_name": "cloned_agent"},
        )
        assert resp.status_code == 401
        ...
        resp = client.post(
            "/api/agents/clone",
            json={"source_agent": "source", "target_name": "auth_cloned"},
            headers=headers,
        )
        assert resp.status_code == 201
