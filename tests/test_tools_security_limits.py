from lemming.tools import FileReadTool, FileWriteTool, MemoryWriteTool, ShellTool
...
def test_shell_tool_dos_prevention(tmp_path):
    tool = ShellTool()

    # Allowed: grep, ls, cat, echo, head, tail, jq

    large_file = tmp_path / "agents" / "test_agent" / "workspace" / "large_file.txt"
    large_file.parent.mkdir(parents=True, exist_ok=True)
    # Create 10MB file
    large_file.write_text("a" * 1024 * 1024 * 10)

    agent_name = "test_agent"

    # Should now fail
    result = tool.execute(agent_name=agent_name, base_path=tmp_path, command="cat large_file.txt")

    assert not result.success
    assert "Output size exceeded" in result.error


def test_file_read_tool_dos_prevention(tmp_path):
    tool = FileReadTool()
    ...
    # Should now fail
    result = tool.execute(agent_name=agent_name, base_path=tmp_path, path="large_file.txt")
