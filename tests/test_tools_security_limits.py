...
def test_file_read_limit(tmp_path):
    ...
    agent_dir = base_path / "agents" / "test_agent"
    workspace_dir = agent_dir / "workspace"
    workspace_dir.mkdir(parents=True)

    tool = FileReadTool()
    max_size = tool.MAX_READ_SIZE

    # Create a file larger than MAX_READ_SIZE
    large_file = workspace_dir / "large.txt"
    large_file.write_bytes(b"A" * (max_size + 10))
    ...
    assert f"Max read size is {max_size} bytes" in result.error
...
def test_file_read_within_limit(tmp_path):
    ...
    # Create a file well within the limit
