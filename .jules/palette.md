...
**Learning:** When elements (like log rows) are only sometimes expandable, applying `role="button"` and `tabIndex={0}` unconditionally creates "phantom buttons" that confuse screen reader users.
**Action:** Conditionally apply `role="button"` and `tabIndex={0}` (and cursor styles) only when the element is actually interactive.
