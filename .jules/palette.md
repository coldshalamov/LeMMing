...
## 2025-05-25 - Conditionally Interactive Elements
**Learning:** When elements (like log rows) are conditionally interactable, unconditionally applying `role="button"` and `tabIndex={0}` creates phantom buttons that confuse screen reader and keyboard users.
**Action:** Always conditionally apply `role`, `tabIndex`, and `aria-expanded` to match the actual interactive state of the element, and ensure keyboard focus states are applied.
