...
**Learning:** Applying unconditional `role="button"` and `tabIndex={0}` to elements like log rows that are only sometimes interactable creates "phantom buttons" for screen reader and keyboard users, leading to confusion and noise.
**Action:** Always conditionally apply `role` and `tabIndex` based on whether the element is actually interactable (e.g., `role={isInteractable ? "button" : undefined}`).
