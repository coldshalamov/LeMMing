...
## 2024-07-27 - Conditional Interactive Roles
**Learning:** When elements (like log rows) are conditionally interactive, unconditionally applying `role="button"` and `tabIndex={0}` creates "phantom buttons" that cause screen reader noise and confuse keyboard navigation.
**Action:** Conditionally apply `role="button"`, `tabIndex={0}`, `aria-expanded`, and pointer cursor CSS classes only when the element is actively interactable.
