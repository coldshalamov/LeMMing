...
## 2025-05-27 - Conditionally Interactive Elements
**Learning:** When elements like log rows are conditionally interactive, hardcoding `role="button"` and `tabIndex={0}` causes screen readers to announce them as "phantom buttons" even when they cannot be interacted with, creating confusing noise.
**Action:** Conditionally apply `role="button"`, `tabIndex={0}`, and `aria-expanded` only when the element is actively interactable (e.g. `role={isInteractable ? "button" : undefined}`).
