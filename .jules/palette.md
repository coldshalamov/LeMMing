...
## 2025-05-26 - Conditional Interactive Roles
**Learning:** Adding `role="button"` and `tabIndex={0}` to all items in a list (like log rows) when only some are interactive creates unnecessary screen reader noise and keyboard navigation stops.
**Action:** Conditionally apply `role="button"` and `tabIndex={0}` (or `undefined`) based on whether the item is actually interactive (e.g., `canExpand`). Ensure non-interactive items remain cleanly skipped.
