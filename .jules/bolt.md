...
## 2024-05-27 - [ModelRegistry Caching]
...
## 2024-05-27 - [JSON Schema Validator Caching]
**Learning:** Repetitive JSON schema validation in `config_validation.py` causes redundant disk reads, JSON parsing, and `Draft7Validator` object instantiation, creating a performance bottleneck during repeated validation operations (like checking configs or resumes repeatedly).
**Action:** Use a module-level dictionary to cache instantiated `Draft7Validator` objects by schema name to bypass redundant I/O and setup overhead in hot paths.
