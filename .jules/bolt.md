...
## 2024-05-27 - [JSON Deserialization and F-String Optimization]
**Learning:** `json.load(f)` is slower than `json.loads(f.read())` when parsing high-throughput JSON. String `.format()` is significantly slower than f-strings.
**Action:** Use `open(..., 'rb')` with `json.loads(f.read())` and prefer f-strings over `.format()` in hot file I/O operations like loading and writing outbox messages.
