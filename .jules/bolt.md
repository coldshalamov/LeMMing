...
## 2024-05-27 - [JSONSchema Validator Caching]
**Learning:** Compiling JSON schemas into validators (e.g., `Draft7Validator(schema)`) is computationally expensive and repetitive when validating many similar objects (like agent resumes).
**Action:** Cache the compiled validator instances in a module-level dictionary rather than re-instantiating them for repeated validations to improve performance.
