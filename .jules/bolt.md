...
## 2026-06-28 - [Draft7Validator Caching]
**Learning:** Initializing Draft7Validator requires reading schema files from disk and parsing JSON. In paths like resume validation (which happens per agent), recreating this validator every time is a significant bottleneck.
**Action:** Cache jsonschema Draft7Validator instances in a module-level dictionary to avoid redundant disk I/O, JSON parsing, and object initialization overheads.
