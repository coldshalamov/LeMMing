...
**Learning:** File reading and JSON parsing (`DepartmentMetadata.from_dict`) on every call to `discover_departments` creates unnecessary overhead, especially considering it iterates over globbed JSON files.
**Action:** Replace `Path.glob` with `os.scandir` to minimize filesystem overhead, handle file checks using `try/except FileNotFoundError` (EAFP), and introduce an in-memory dictionary caching mechanism mapping `entry.path` to `(entry.stat().st_mtime, copy.deepcopy(dept))` to avoid repetitive reads. Ensure cache data is robustly copied on return to avoid state poisoning.
