...
## 2024-05-27 - [Batching disk I/O in tick loops]
**Learning:** In the LeMMing file-based architecture, when updating shared state across multiple agents in a single tick (like credits), doing it incrementally per agent causes severe O(N) disk I/O bottlenecks.
**Action:** Batch disk I/O write operations at the end of the tick loop (e.g. inside `run_tick`) rather than writing incrementally per agent.
