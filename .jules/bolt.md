...
## 2026-05-31 - [JSON Parsing Mode Performance]
**Learning:** In high-throughput I/O pathways, using `open(..., "rb")` with `json.loads(f.read())` is noticeably faster than text-mode `json.load(f)` because it bypasses Python's standard library text decoding overhead.
**Action:** Use binary mode reading combined with `json.loads()` for frequent JSON file parsing.
