...
## 2026-06-14 - [jsonschema Validator Caching]
**Learning:** Compiling `jsonschema` schemas into `Draft7Validator` instances is computationally expensive (~16ms). Repeated validations re-read and recompile the identical schemas repeatedly.
**Action:** Cache the compiled validator instances (e.g., in a module-level dictionary) instead of re-instantiating them for every validation call to gain an order of magnitude speedup.
