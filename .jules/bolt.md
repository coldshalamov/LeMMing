...
## 2026-05-22 - [Optimizing Outbox Parsing]
**Learning:** Checking timestamps for outbox processing by loading full JSON is slow. Outbox filenames start with the tick (e.g., `123_msg.json`). By using `os.scandir` and partitioning the filename string, we can filter out old entries without expensive file reads and JSON deserialization.
**Action:** Always extract `tick` from outbox filenames instead of reading file contents when checking age.
