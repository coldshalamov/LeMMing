...
## 2024-05-28 - [Social Graph Analysis Optimization]
**Learning:** In `analyze_social_graph`, iterating over a list of relationships to find a match for each outbox entry results in an O(N^2) operation. Furthermore, using `Path.glob()` and fully instantiating `OutboxEntry` objects for every file adds significant file I/O and parsing overhead.
**Action:** Pre-index the list of relationships into a dictionary for O(1) lookups. Use `os.scandir` instead of `Path.glob()` and avoid fully parsing JSON into dataclasses when only specific fields are needed.
