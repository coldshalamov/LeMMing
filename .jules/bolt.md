...
## 2026-06-15 - [JSON Schema Validation Caching]
**Learning:** Instantiating `jsonschema.Draft7Validator` and repeatedly loading the schema file from disk is a heavy operation, significantly impacting performance in loops like `validate_everything`.
**Action:** Implemented a module-level cache mapping schema filenames to their compiled `Draft7Validator` instances to eliminate redundant file IO and object creation.
