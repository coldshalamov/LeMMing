...
## 2024-07-27 - O(N^2) Nested Loop in JSON Aggregation
**Learning:** Iterating through an unindexed list of relations inside a tight loop processing many JSON file entries severely degrades performance as total interaction counts and agent lists grow in the backend.
**Action:** Build O(1) dictionary indices mapping relationship pairs immediately before parsing directories of JSON objects to avoid unnecessary multiple iterations over metadata. Prefer `os.scandir` combined with `try-except OSError` blocks over `pathlib.Path.glob` and `exists` checks in I/O heavy iterations.
