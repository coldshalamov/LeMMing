...
## 2026-06-10 - [Optimize social graph analysis]
**Learning:** `analyze_social_graph` reads all outbox entries globally with `.glob("*.json")`, opens them, and parses them using `json.load` and `OutboxEntry.from_dict` sequentially. In environments with many old outbox files, fully parsing JSON and discarding it just for filtering `tick < recent_tick_threshold` adds substantial overhead.
**Action:** Replace `glob` with `os.scandir` to skip instantiating redundant `Path` objects, use `partition` to extract the tick directly from the filename to bypass parsing `json.load` for old files, and only access specific fields from the loaded JSON dict instead of `OutboxEntry.from_dict` when data structures don't strictly require instantiation.
