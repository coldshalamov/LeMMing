...
## 2024-05-27 - [Optimizing JSON Loading in Hot Paths]
**Learning:** Using `open(..., "rb")` with `json.loads(f.read())` is noticeably faster for parsing JSON than the standard text mode `json.load(f)`, especially in I/O-heavy paths like memory or outbox operations.
**Action:** For high-throughput internal JSON files (outbox, memory, resume.json), switch to binary mode reading and string-based parsing to save overhead.
