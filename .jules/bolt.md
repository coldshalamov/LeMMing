...
## 2026-06-27 - [Draft7Validator Caching]
**Learning:** Initializing `jsonschema.Draft7Validator` repeatedly for the same schema causes redundant disk I/O (reading schema file) and JSON parsing overhead, which becomes a bottleneck when validating many items (like agent resumes).
**Action:** Cache `Draft7Validator` instances in a module-level dictionary to avoid redundant object initialization and file I/O overheads.
