...
## 2024-05-28 - [Batching Disk I/O]
**Learning:** In the file-based architecture, updating shared state (like credits) incrementally per agent in a single tick causes O(N) disk writes.
**Action:** Batch disk I/O write operations at the end of the tick loop (e.g., conditionally saving credits if agents fired) to eliminate disk bottlenecks.
