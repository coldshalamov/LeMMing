...
## 2026-06-06 - [Cache jsonschema Draft7Validator Compilation]
**Learning:** Compiling schemas into validators in the `jsonschema` library (e.g., `Draft7Validator(schema)`) is computationally expensive and slow when validating items inside loops.
**Action:** Cache the compiled validator instances in a module-level dictionary rather than re-instantiating them for repeated validations.
