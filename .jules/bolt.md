...
## 2026-09-13 - [Optimizing load_agent Caching]
...
## 2026-09-13 - [ModelRegistry Caching]
...
## 2026-09-13 - [Batched Write for Credits]
**Learning:** `deduct_credits` is called in the hot path of `run_agent` causing an O(N) disk I/O bottleneck when multiple agents fire in a single tick as they all attempt to save to `credits.json` independently.
**Action:** In the file-based architecture, when updating shared state across multiple agents in a single tick, batch disk I/O write operations at the end of the tick loop (e.g., within `run_tick`) rather than writing incrementally per agent. Ensure checking if the list of processed entities is not empty before the batched write to avoid redundant operations on empty ticks.
