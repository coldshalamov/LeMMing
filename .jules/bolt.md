...
## 2026-06-18 - [Cache jsonschema Draft7Validator instances]
**Learning:** The validation uses `Draft7Validator` from `jsonschema` dynamically by loading and parsing schema files repeatedly. Repeatedly instantiating the validator and parsing schema files from disk introduces an unnecessary overhead during configuration validation, adding up significantly over repeated checks.
**Action:** Used a dictionary to cache the initialized `Draft7Validator` objects keyed by `schema_name` to prevent redundant disk I/O, parsing of JSON schemas, and initializing `Draft7Validator` instances on every validation step.
