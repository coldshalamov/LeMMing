...
**Learning:** Instantiating config registries (like `ModelRegistry`) that run JSON schema validations (`jsonschema.Draft7Validator`) on every call can introduce significant hidden overhead (~0.75ms per call) on hot paths like LLM interactions.
**Action:** Use a module-level dictionary cache keyed by the config directory to store and reuse registry instances, preventing redundant disk reads and schema validations on repeated function calls.
