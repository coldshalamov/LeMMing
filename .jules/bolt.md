...
## 2026-06-01 - [JSONSchema Validator Caching]
**Learning:** Compiling `jsonschema` schemas into `Draft7Validator` instances is computationally expensive and causes a bottleneck during repeated validations (like parsing agent `resume.json` files).
**Action:** Cache the compiled validator instances (e.g., in a module-level dictionary) instead of re-instantiating them for each validation call to significantly improve performance.
