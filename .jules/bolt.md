...
## 2026-06-04 - [Draft7Validator Caching]
**Learning:** When using the `jsonschema` library, compiling schemas into validators (e.g., `Draft7Validator(schema)`) is computationally expensive and adds overhead when called repeatedly.
**Action:** Cache the compiled validator instances (e.g., in a module-level dictionary) rather than re-instantiating them for repeated validations to improve performance.
