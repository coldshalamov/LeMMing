...
## 2026-06-08 - [Draft7Validator Caching]
**Learning:** Compiling schemas into validators using the `jsonschema` library is computationally expensive. Repeatedly compiling the same schema during frequent validations (like loading multiple agents or configurations) introduces significant overhead.
**Action:** Cache the compiled `Draft7Validator` instances in a module-level dictionary to reuse them across multiple validations, reducing the execution time.
