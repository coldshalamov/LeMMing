...
## 2026-05-07 - [JSON Parsing Performance]
**Learning:** Python's `json.load(f)` on text mode files is noticeably slower than reading raw bytes and using `json.loads(f.read())`.
**Action:** In high-throughput I/O pathways, use `open(..., 'rb')` with `json.loads(f.read())` instead of text mode `json.load(f)` for faster JSON parsing.
