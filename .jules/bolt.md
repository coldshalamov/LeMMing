...
## 2026-05-30 - [Draft7Validator Caching]
**Learning:** Compiling `jsonschema` schemas into `Draft7Validator` instances is computationally expensive and slow when repeated frequently.
**Action:** Cache the compiled validator instances in a module-level dictionary instead of re-instantiating them for each validation to significantly improve configuration/resume validation performance.
