...
## 2024-05-27 - [Batching Agent Credit Persistence]
**Learning:** During tick execution, credit deductions for each agent trigger an immediate O(N) disk write by saving `credits.json` repeatedly.
**Action:** Batch the file I/O operations by deducting credits in memory during agent loops, and persisting to disk via `save_credits` only once at the end of the tick.
