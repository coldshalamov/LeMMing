...
## 2026-05-15 - [Faster JSON Parsing in Hot Paths]
**Learning:** For high-throughput I/O pathways like reading outbox entries, using `json.load(f)` with text-mode files is noticeably slower due to decoding overhead. Using `open(..., "rb")` with `json.loads(f.read())` is about 20% faster.
**Action:** When working in hot loops for file I/O operations like loading cached outbox entries or memory files, use `open(..., 'rb')` and `json.loads(f.read())` instead of `json.load(f)`.
