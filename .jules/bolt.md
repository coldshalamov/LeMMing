...
## 2026-05-28 - [Fast JSON Parsing in Hot Paths]
**Learning:** In high-throughput I/O pathways in Python, reading a file in binary mode (`open(..., 'rb')`) and parsing the entire content with `json.loads(f.read())` is significantly faster than using text mode `json.load(f)`.
**Action:** When working on performance-critical file I/O operations (like reading outbox entries), use binary reading and `json.loads` instead of `json.load`.
