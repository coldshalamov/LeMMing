...
## 2026-09-11 - [Batching Shared State Disk Writes]
**Learning:** In the file-based architecture, when updating shared state across multiple agents in a single tick (e.g., credits), writing incrementally per agent creates an O(N) disk I/O bottleneck.
**Action:** Batch disk I/O write operations at the end of the tick loop (e.g., within `run_tick`) conditionally checking if the list of processed entities is not empty.
