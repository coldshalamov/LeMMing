...
## 2024-07-27 - Environment Variable Leak in CLIProvider
**Vulnerability:** The `CLIProvider` directly copied and merged `os.environ` into the environment passed to `subprocess.run()`, exposing all parent environment variables (including sensitive keys like `API_KEY`, `LEMMING_ADMIN_KEY`, and `OPENAI_API_KEY`) to the spawned child process.
**Learning:** Child processes inherit the full parent environment by default or when explicitly passed via `os.environ.copy()`. If the parent environment contains sensitive secrets, untrusted child processes can easily access them.
**Prevention:** Always explicitly sanitize the environment dictionary (e.g., filtering out keys containing `API_KEY`, `TOKEN`, `SECRET`, or `PASSWORD`) before passing it to `subprocess.run()` when executing external commands. Ensure sanitization happens *before* merging explicitly provided configurations to prevent dropping intentional secrets.
