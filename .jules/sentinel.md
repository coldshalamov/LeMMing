...
## 2024-05-29 - Security Module Shadowing
**Vulnerability:** A local variable was named `secrets = json.load(...)`, shadowing the built-in `secrets` python module. This caused an `AttributeError` when attempting to call `secrets.compare_digest` in the `verify_admin_access` auth check, effectively breaking the authentication.
**Learning:** Naming configuration or payload variables the same as standard security modules is a subtle way to inadvertently break security checks. When the standard module is overridden in scope, critical cryptographic functions crash.
**Prevention:** Avoid naming local variables `secrets`, `hashlib`, etc. in environments that import these modules. Always use descriptive names like `loaded_secrets` or `config_secrets`.
