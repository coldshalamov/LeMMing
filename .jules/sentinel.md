...
## 2024-05-25 - Python CI Failures
**Vulnerability:** Not a security vulnerability.
**Learning:** Fixing general Python bugs (like variable shadowing of standard library modules causing `mypy` errors) and linting issues is generally out of scope for Sentinel, but when the codebase has extensive preexisting linter issues that break the CI pipeline, they must be fixed to allow the CI to pass.
**Prevention:** Always run linters and fix their output before pushing.
