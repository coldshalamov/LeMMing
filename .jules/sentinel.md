...
## 2024-05-29 - Environment Variable Leakage to Subprocesses
**Vulnerability:** The `ShellTool` and `CLIProvider` passed the full host environment to `subprocess.run`, unintentionally exposing highly sensitive credentials (like `OPENAI_API_KEY` or custom user secrets) to unverified or potentially malicious underlying shell commands.
**Learning:** By default, `subprocess.run` without an `env` argument (or with `os.environ.copy()`) inherits and leaks the entire environment of the parent Python process to the child process.
**Prevention:** Always sanitize the environment mapping by actively filtering out sensitive key patterns (e.g., `API_KEY`, `TOKEN`, `SECRET`, `PASSWORD`) before passing the explicit `env` dictionary to any untrusted subprocess execution context.
