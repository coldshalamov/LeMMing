...
**Vulnerability:** `subprocess.run()` calls in `CLIProvider` and `ShellTool` implicitly inherited the full parent environment, leaking sensitive API keys and secrets to executed CLI commands.
**Learning:** Child processes inherit the parent's environment by default. If the parent holds sensitive keys (e.g., in `os.environ`), any spawned tool can access them.
**Prevention:** Always explicitly pass a sanitized `env` dictionary to `subprocess.run()`, filtering out sensitive keys before merging any explicit configurations.
