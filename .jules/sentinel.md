...
## 2024-05-29 - Missing Authentication on Message Sending Endpoint
**Vulnerability:** The `/api/messages` POST endpoint, which allows sending messages (mutating state), lacked the `verify_admin_access` dependency. This allowed unauthenticated users to inject messages into the system.
**Learning:** When adding new state-mutating endpoints, it is easy to forget to apply the global authentication middleware or dependencies, especially when rate limiting is present and might give a false sense of security.
**Prevention:** Always audit newly added endpoints for proper authorization dependencies (e.g., `Depends(verify_admin_access)` in FastAPI) before merging, particularly those that change the system's state or behavior.
