...
## 2024-05-29 - Unsecured Write Endpoint in "Opt-In" Auth System
**Vulnerability:** The `send_message` endpoint lacked the authentication dependency used by other sensitive endpoints (`verify_admin_access`), allowing unauthorized users to trigger agent actions when `LEMMING_ADMIN_KEY` was set.
**Learning:** When implementing "opt-in" or partial authentication, it's easy to miss endpoints that seem "operational" (triggering behavior) but not "configuration". Every state-mutating endpoint must be audited against the optional auth mechanism.
**Prevention:** Centralize route definition or use a router-level dependency for all "protected" routes instead of applying `Depends` per-function, which is error-prone.
