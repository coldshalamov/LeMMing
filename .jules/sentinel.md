...
## 2024-09-06 - Cross-Platform Absolute Path Bypass
**Vulnerability:** `Path(arg).is_absolute()` only uses the host OS path parsing rules, meaning an attacker could bypass absolute path checks by providing Windows paths (`C:\etc\passwd`) on Unix environments or vice versa.
**Learning:** Pathlib uses host-specific rules for `Path`. Security boundaries protecting against arbitrary path access need to enforce path structure constraints universally regardless of the host OS.
**Prevention:** Use both `PurePosixPath(arg).is_absolute() or PureWindowsPath(arg).is_absolute()` to prevent cross-platform absolute path traversal bypasses.
