...
## 2024-07-27 - Subprocess Environment Variable Leakage
**Vulnerability:** ShellTool and CLIProvider used `subprocess.run` which inherited the full parent environment (`os.environ`), leaking sensitive API keys and secrets to child processes.
**Learning:** Child processes implicitly inherit the parent environment unless overridden. In Python, omitting the `env` argument to `subprocess.run` (or passing `os.environ.copy()` without filtering) exposes all secrets.
**Prevention:** Always explicitly pass a sanitized `env` dictionary to `subprocess.run` by copying `os.environ` and removing sensitive keys (like those containing 'API_KEY', 'TOKEN', 'SECRET', 'PASSWORD') before merging any custom configuration.
