...
## 2024-05-29 - Insecure Secrets File Permissions
**Vulnerability:** The API saved the `secrets.json` file with default permissions, which may allow other users on the system to read API keys.
**Learning:** Using `open()` to write a file does not enforce restrictive permissions on newly created or modified files.
**Prevention:** Always use `os.chmod()` with restrictive permissions (like `0o600`) after writing sensitive files to ensure they are properly secured against local privilege escalation.
