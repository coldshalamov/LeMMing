...
## 2024-05-29 - Argument Injection Bypass via Leading Whitespace
**Vulnerability:** The CLI wrapper's Argument Injection check `prompt.startswith("-")` was bypassable by prepending spaces (e.g., ` -flag`).
**Learning:** Checking prefixes strictly requires accounting for variable whitespace input.
**Prevention:** Always strip leading whitespaces using `lstrip()` before verifying a prefix, e.g. `prompt.lstrip().startswith("-")`, to avoid bypassing the check.
