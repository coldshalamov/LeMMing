...
## 2024-04-27 - Fix missing WebSocket endpoint authentication
**Vulnerability:** The `/ws` endpoint in `lemming/api.py` lacked authentication, broadcasting sensitive system statuses and messages to any client, even when `LEMMING_ADMIN_KEY` was configured.
**Learning:** When securing endpoints in a REST API, WebSocket endpoints are often overlooked because standard HTTP middlewares or custom header checks (`X-Admin-Key`) don't naturally apply to standard browser `WebSocket` connection requests.
**Prevention:** Always verify authentication for WebSocket endpoints via connection payloads, subprotocols, or query parameters (`?admin_key=...`) prior to `websocket.accept()`.
