...
## 2024-05-29 - Unauthenticated WebSocket Endpoints
**Vulnerability:** The `/ws` endpoint was implemented without any authentication, bypassing the `LEMMING_ADMIN_KEY` protection used for HTTP endpoints. This allowed unauthorized access to real-time status and message streams.
**Learning:** WebSocket endpoints often bypass standard HTTP middleware or dependencies if not explicitly secured. Standard `Authorization` headers are also not supported by the browser `WebSocket` API, requiring alternative transmission methods (query params or protocols).
**Prevention:** Always apply authentication dependencies to WebSocket endpoints. For browser compatibility, support passing credentials via query parameters (e.g. `?key=...`) or subprotocols, and validate them during the handshake.
