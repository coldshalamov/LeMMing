...
## 2024-05-29 - Cross-Platform Absolute Path Validation Bypass
**Vulnerability:** ShellTool validated absolute paths using `Path(arg).is_absolute()`. Because `Path` uses the host OS semantics, this allowed an attacker to supply a Windows absolute path (e.g., `C:\Windows`) to a LeMMing instance hosted on Unix, bypassing the security check.
**Learning:** `Path.is_absolute()` is OS-dependent. When securing sandboxed inputs that might be crafted by remote users (or LLMs) who are unaware of the host OS, you must assume paths could follow any OS convention.
**Prevention:** Use `pathlib.PurePosixPath(arg).is_absolute() or pathlib.PureWindowsPath(arg).is_absolute()` to enforce strict, cross-platform absolute path blocking regardless of the environment the application is running in.
