...
## 2024-05-29 - Module Shadowing in Authentication Logic
**Vulnerability:** The `secrets` local variable in `lemming/api.py` shadowed the Python standard library `secrets` module. This caused the `verify_admin_access` function to crash with an `AttributeError` when `secrets.json` was present, effectively denying admin access.
**Learning:** Python's scoping rules allow local variables to shadow global imports silently. In security-critical code, this can lead to availability vulnerabilities or fallback to insecure methods.
**Prevention:** Use distinct variable names for configuration data (e.g., `local_secrets`) to avoid conflicting with standard library modules.
