...
## 2026-05-31 - Argument Injection Bypass via Whitespace
**Vulnerability:** The `CLIProvider` argument injection protection only checked if the prompt strictly started with `-`. Attackers could bypass this by prefixing their payload with a space (e.g., ` -flag`), leading to argument injection.
**Learning:** Basic string prefix checks (`startswith`) are insufficient for security validation when parsers or CLI tools automatically strip leading whitespace from arguments.
**Prevention:** Always strip leading whitespace (`lstrip()`) before validating prefixes to ensure the validation matches the underlying tool's interpretation.
