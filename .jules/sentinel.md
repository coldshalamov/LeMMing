...
## 2024-05-29 - Shadowing Built-in Secrets Module
**Vulnerability:** The built-in `secrets` module was inadvertently shadowed by a local variable dictionary named `secrets`.
**Learning:** Avoid using variables with the same name as built-in modules, particularly in the global scope. This caused `secrets.compare_digest()` to fail with an `AttributeError`.
**Prevention:** Use descriptive local variable names (e.g., `loaded_secrets`) to avoid shadowing system modules.
