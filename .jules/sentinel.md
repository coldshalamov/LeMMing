...
## 2024-05-29 - Missing Authorization on Agent Messages Endpoint
**Vulnerability:** The `/api/messages` endpoint lacked authentication dependencies, allowing unauthenticated users to inject messages into the agent system and consume credits or influence state even when the system was secured with `LEMMING_ADMIN_KEY`.
**Learning:** In API-driven systems, input injection points (like messages from external sources) are just as critical to secure as configuration or creation endpoints because they drive the state machine and consume resources.
**Prevention:** All state-mutating endpoints, especially those that trigger downstream processing or external API calls (like LLM generations), must explicitly include authorization dependencies.
