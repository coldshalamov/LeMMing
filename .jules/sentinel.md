...
## 2025-01-20 - Ensure sensitive configuration files have restrictive permissions

**Vulnerability:** The `secrets.json` file generated by `update_engine_config` lacked restrictive permissions, allowing local privilege escalation or information disclosure if other users on the system read the file.
**Learning:** Local files storing sensitive credentials must be explicitly secured. `os.open` with restrictive flags doesn't retroactively fix files if they already exist with weak permissions.
**Prevention:** Explicitly call `os.chmod(path, stat.S_IRUSR | stat.S_IWUSR)` after writing sensitive data to secure both new and existing files.
