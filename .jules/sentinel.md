...
## 2024-06-20 - Subprocess Environment Leak
**Vulnerability:** The `subprocess.run` calls in `CLIProvider` and `ShellTool` implicitly inherited `os.environ` or copied it without sanitization, leaking sensitive backend credentials (e.g., `OPENAI_API_KEY`) to untrusted shell commands and CLI wrappers.
**Learning:** If the `env` argument is omitted in `subprocess.run`, it defaults to inheriting the entire parent environment. When wrapping untrusted execution, the environment must be explicitly defined and sanitized.
**Prevention:** Always explicitly pass a sanitized `env` dictionary to `subprocess.run` (e.g., removing keys containing `API_KEY` or `TOKEN`) when executing shell commands that may be influenced by untrusted actors.
