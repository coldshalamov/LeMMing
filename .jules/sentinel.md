...
## 2024-05-29 - Module Shadowing Vulnerability
**Vulnerability:** The built-in `secrets` module was shadowed by naming a configuration dictionary `secrets` (e.g., `secrets = json.load(f)`).
**Learning:** Shadowing standard modules with local variables can cause unexpected crashes in security-critical functions like `secrets.compare_digest`, leading to authentication failure (500 errors instead of proper validation).
**Prevention:** Avoid using standard library module names (like `secrets`, `os`, `sys`) as variable names, especially when those modules are imported and used in the same scope.
