...
class CLIProvider(LLMProvider):
    ...
    def call(self, model_name: str, messages: list[dict[str, str]], temperature: float = 0.2, **kwargs: Any) -> str:
        ...
        try:
            ...
            # Sanitize environment to prevent leaking secrets to subprocesses
            sanitized_env = {}
            for k, v in run_env.items():
                k_upper = k.upper()
                if not any(sec in k_upper for sec in ["API_KEY", "TOKEN", "SECRET", "PASSWORD"]):
                    sanitized_env[k] = v
            run_env = sanitized_env
