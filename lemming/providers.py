...
class CLIProvider(LLMProvider):
    ...
    def call(self, model_name: str, messages: list[dict[str, str]], temperature: float = 0.2, **kwargs: Any) -> str:
        ...
        try:
            ...
            # Security fix: Prevent leaking parent secrets to child processes
            for k in list(run_env.keys()):
                if any(sec in k.upper() for sec in ["API_KEY", "TOKEN", "SECRET", "PASSWORD"]):
                    run_env.pop(k, None)

            if self.env:
                # Explicitly provided env vars take precedence and are allowed
