...
class CLIProvider(LLMProvider):
    ...
    def __init__(self, command: str | list[str], prompt_strategy: str = "append", cwd: str | None = None, env: dict[str, str] | None = None, timeout: float = 60.0, prevent_arg_injection: bool = True):  # noqa: E501
    ...
    def call(self, model_name: str, messages: list[dict[str, str]], temperature: float = 0.2, **kwargs: Any) -> str:
        ...
        if self.prevent_arg_injection and prompt.startswith("-"):
            ...
            raise ValueError(f"Security violation: Prompt '{prompt}' starts with '-' which could be interpreted as a flag. "  # noqa: E501
        ...
        logger.info(f"Executing CLI command: {cmd_args}")
        ...
        try:
            ...
            return self._wrap_output(f"Error executing CLI: {e}")

    def _wrap_output(self, raw_output: str) -> str:
        """
        Try to parse as JSON, otherwise wrap in the expected LeMMing JSON format.
        """
        try:
            # If the CLI tool already outputs JSON that matches our schema, return it.
            # This is optimistic.
...
def get_provider(provider_name: str, **kwargs: Any) -> LLMProvider:
    ...
    Args:
        provider_name: Name of the provider (openai, anthropic, ollama, cli)
