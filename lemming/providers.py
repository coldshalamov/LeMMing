...
class CLIProvider(LLMProvider):
    ...
    def call(self, model_name: str, messages: list[dict[str, str]], temperature: float = 0.2, **kwargs: Any) -> str:
        ...
        try:
            # Merge env and sanitize sensitive variables to prevent leakage to child processes
            run_env = os.environ.copy()
            for key in list(run_env.keys()):
                if any(secret in key.upper() for secret in ("API_KEY", "TOKEN", "SECRET", "PASSWORD")):
