...
class CLIProvider(LLMProvider):
    ...
    def call(self, model_name: str, messages: list[dict[str, str]], temperature: float = 0.2, **kwargs: Any) -> str:
        ...
        try:
            ...
            # Security: Sanitize environment variables before passing to subprocess
            # to prevent leaking sensitive API keys to the CLI tool
            for key in list(run_env.keys()):
                upper_key = key.upper()
                if any(
                    secret_indicator in upper_key for secret_indicator in ["API_KEY", "TOKEN", "SECRET", "PASSWORD"]
                ):
                    del run_env[key]
