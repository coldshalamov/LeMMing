...
class ShellTool(Tool):
    ...
    def execute(self, agent_name: str, base_path: Path, **kwargs: Any) -> ToolResult:
        ...
        for arg in args[1:]:
            # Security check: Prevent Argument Injection in sandboxed tools.
            # We explicitly block any argument that starts with a dash to prevent bypassing
            # absolute path restrictions via arguments like `--file=/etc/passwd`
            if arg.startswith("-"):
                return ToolResult(
                    False,
                    "",
                    f"Security violation: Argument injection detected. Flags starting with '-' are not allowed: {arg}",
                )
