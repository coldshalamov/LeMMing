...
class FileReadTool(Tool):
    ...
    def execute(self, agent_name: str, base_path: Path, **kwargs: Any) -> ToolResult:
        ...
        # Security check: prevent reading excessively large files (DoS protection)
        try:
            if target_path.stat().st_size > self.MAX_READ_SIZE:
                return ToolResult(
                    False,
                    "",
                    f"File too large to read. Max size is {self.MAX_READ_SIZE} bytes.",
                )
        except OSError as e:
            return ToolResult(False, "", f"Failed to get file size: {e}")
