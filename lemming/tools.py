        ...
        try:
            ...
            if size > self.MAX_READ_SIZE:
                return ToolResult(
                    False,
                    "",
                    f"File too large ({size} bytes). Max size is {self.MAX_READ_SIZE} bytes.",
                )
        except OSError as e:
...
class FileListTool(Tool):
    ...
    def execute(self, agent_name: str, base_path: Path, **kwargs: Any) -> ToolResult:
        ...
        allowed_workspace = workspace_dir.resolve()
        allowed_shared = (base_path / "shared").resolve()

        if not (target_path.is_relative_to(allowed_workspace) or target_path.is_relative_to(allowed_shared)):
            return ToolResult(False, "", "Security violation: path is outside allowed directories")
