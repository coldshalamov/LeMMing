...
class FileListTool(Tool):
    ...
    def execute(self, agent_name: str, base_path: Path, **kwargs: Any) -> ToolResult:
        ...
        # Resolve all paths to ensure robust checking against traversals
        resolved_workspace = workspace_dir.resolve()
        resolved_shared = (base_path / "shared").resolve()

        if not (target_path.is_relative_to(resolved_workspace) or target_path.is_relative_to(resolved_shared)):
            return ToolResult(False, "", "Security violation: path is outside allowed directories")
