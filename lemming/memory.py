...
def load_memory(base_path: Path, agent_name: str, key: str) -> Any | None:
    ...
    try:
        # Optimization: use 'rb' mode with json.loads(f.read()) instead of 'r' with json.load(f)
        # for ~26% faster JSON parsing in high-throughput paths
            ...
            for entry in it:
                if entry.is_file() and entry.name.endswith(".json"):
                    ...
                    try:
                        ...
                        # Optimization: json.loads(f.read()) on 'rb' mode is faster than json.load()
