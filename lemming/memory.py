...
def save_memory(base_path: Path, agent_name: str, key: str, value: Any, operation: str = 'set', tick: int | None = None) -> None:  # noqa: E501
    """
    ...
    entry = {'key': key, 'value': value, 'timestamp_utc': datetime.now(UTC).isoformat(), 'agent': agent_name, 'operation': operation, 'tick': tick}  # noqa: E501
...
def get_memory_summary(base_path: Path, agent_name: str) -> dict[str, Any]:
    ...
    # Optimization: Use os.scandir to avoid creating Path objects and multiple stat calls.
    # This replaces the N+1 pattern of list_memories() followed by load_memory() for each key.
    try:
        with os.scandir(memory_dir) as it:
            for entry in it:
                if entry.is_file() and entry.name.endswith(".json"):
