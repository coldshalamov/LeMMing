...
def rate_limiter(limit: int = 10, window: int = 60, scope: str = "default"):
...
@app.post(
    "/api/agents",
    status_code=201,
    dependencies=[Depends(rate_limiter(limit=5, window=60, scope="create_agent")), Depends(verify_admin_access)],
)
...
@app.post(
    "/api/agents/clone",
    status_code=201,
    dependencies=[Depends(rate_limiter(limit=5, window=60, scope="clone_agent")), Depends(verify_admin_access)],
)
...
@app.post(
    "/api/engine/tick",
    dependencies=[Depends(rate_limiter(limit=10, window=60, scope="tick")), Depends(verify_admin_access)],
)
