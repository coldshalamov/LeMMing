...
if SECRETS_PATH.exists():
    try:
        with open(SECRETS_PATH) as f:
            # Sentinel: Use _secrets_dict instead of 'secrets' to avoid shadowing the built-in module
            _secrets_dict = json.load(f)
            for k, v in _secrets_dict.items():
                if v and not os.environ.get(k):
                    os.environ[k] = v
            # Sentinel: Delete from namespace so no sensitive dict data is left around
            del _secrets_dict
    except Exception:
        pass
