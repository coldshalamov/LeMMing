...
async def verify_websocket_access(websocket: WebSocket, key: str | None = None):
    """Verify admin access for WebSocket connections."""
    admin_key = os.environ.get("LEMMING_ADMIN_KEY")
    if not admin_key:
        return
    ...
    if not candidate_key:
        candidate_key = websocket.headers.get("X-Admin-Key")

    if not candidate_key or not secrets.compare_digest(candidate_key, admin_key):
...
@app.websocket("/ws")
async def websocket_endpoint(websocket: WebSocket, _: None = Depends(verify_websocket_access)) -> None:
