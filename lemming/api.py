...
def _read_agent_logs(base_path: Path, agent_name: str, limit: int = 100) -> list[dict[str, Any]]:
    ...
    # ⚡ Bolt Optimization: Use _read_last_lines to efficiently read only the
    # required lines from the end of the file, avoiding loading and splitting
    # up to 1MB of data into memory when only 'limit' lines are needed.
    # This prevents O(N) memory allocation and splitlines() overhead.
