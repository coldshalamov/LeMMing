...
def run_agent(base_path: Path, agent: Agent, tick: int) -> dict[str, Any]:
    ...
    # Deduct credits
    # Optimization: Defer disk write (save=False) to batch save_credits at end of tick.
    # Impact: Reduces O(N) I/O writes to O(1) for shared states per tick loop.
...
def run_tick(base_path: Path, tick: int) -> dict[str, Any]:
    ...
    if firing_agents:
        # Optimization: Batch save shared states once per tick to avoid O(N) I/O bottleneck
