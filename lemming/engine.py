...
def run_agent(base_path: Path, agent: Agent, tick: int, persist_credits: bool = True) -> dict[str, Any]:
    ...
    try:
        raw_output = call_llm(
            agent.model.key,
            prompt,
            temperature=agent.model.temperature if agent.model.temperature is not None else 0.2,
    ...
    # Deduct credits
    deduct_credits(agent.name, cost_per_action, base_path, persist=persist_credits)
...
def run_tick(base_path: Path, tick: int) -> dict[str, Any]:
    ...
    try:
        for agent in firing_agents:
            ...
            # Optimization: don't persist credits for every agent; do it once at end of tick
            results[agent.name] = run_agent(base_path, agent, tick, persist_credits=False)
    finally:
        if firing_agents:
