...
def run_agent(base_path: Path, agent: Agent, tick: int) -> dict[str, Any]:
    ...
    # Deduct credits
    # ⚡ Bolt: Pass save=False to avoid immediate disk I/O, deferring to the batched save at the end of the tick.
...
def run_tick(base_path: Path, tick: int) -> dict[str, Any]:
    ...
    if firing_agents:
        # ⚡ Bolt: Batched save here reduces O(N) disk writes per tick to O(1)
