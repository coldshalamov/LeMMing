  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label={
          isCollapsed ? "Expand manager chat" : "Collapse manager chat"
        }
        tabIndex={0}
        onKeyDown={(e) => {
      ...
      <AnimatePresence>
        {!isCollapsed && (
          ...
          >
                  ...
                  <div className="flex flex-col gap-2 w-full">
                    {SUGGESTED_PROMPTS.map((prompt) => (
                      ...
                      >
                        <Sparkles
                          size={12}
                          className="text-brand-purple opacity-50 group-hover:opacity-100 transition-opacity"
                        />
                        {prompt}
                      </button>
                    ))}
            ...
            <form
              onSubmit={handleSend}
              className="p-3 bg-black/40 border-t border-white/5 flex gap-2 items-end"
            >
              <textarea
                ref={textareaRef}
                value={inputValue}
              ...
              <button
                type="submit"
                onClick={(e) => {
                  if (!inputValue.trim() || isSending) e.preventDefault();
                }}
                ...
                className={clsx(
                  "p-2 bg-brand-purple text-white rounded-lg hover:bg-purple-600 transition-colors h-[38px] flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black",
                  (!inputValue.trim() || isSending) && "opacity-50",
                  isSending
                    ? "cursor-wait"
