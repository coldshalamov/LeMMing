...
import { useAgents, useOrgGraph, useStatus, useMessages, useWebSocketStream, triggerTick } from "@/lib/api";
...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col h-screen bg-neo-bg text-foreground font-sans overflow-hidden relative">
      ...
      <div className="absolute inset-0 pointer-events-none z-10 flex flex-col justify-between">
        ...
        <header className="flex items-center justify-between px-6 pt-4">
          ...
          <div className="pointer-events-auto bg-black/40 backdrop-blur-md px-4 py-2 rounded-lg border border-white/5 flex items-center gap-6 text-xs font-mono text-gray-400">
            ...
            <div className={clsx("flex items-center gap-2", isConnected ? "text-brand-cyan" : "text-red-400")}>
        ...
        <div className="flex justify-center pb-8 pointer-events-auto">
          <div className="flex items-center gap-4 p-2 pl-6 pr-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl">
            <div className="flex flex-col">
              <span className="text-[10px] font-mono text-brand-lime uppercase tracking-widest">System Status</span>
              <span className="text-sm font-bold text-white">{isTicking ? "EXECUTING_TICK..." : "READY_TO_EXECUTE"}</span>
            </div>
            ...
            <button
              ...
              className={clsx(
                ...
                isTicking ? "bg-gray-600 text-gray-400" : "bg-brand-lime text-black shadow-[0_0_20px_rgba(132,204,22,0.4)]"
              )}
            >
              {isTicking ? <Loader2 size={24} className="animate-spin" /> : <Play fill="currentColor" className="ml-1" size={24} />}
            </button>
          </div>
        </div>
      </div>
      ...
      {/* 3. LAYER: AGENT OVERLAY (Top Right) */}
            ...
            >
              ...
              <div className="p-4 border-b border-white/5 flex justify-between items-start bg-black/20">
                <div>
                  ...
                  <p className="text-xs text-brand-cyan font-mono uppercase">{selectedAgent.title}</p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="rounded border border-white/5 bg-black/20 p-3">
                  <h4 className="text-[10px] font-mono text-white/40 uppercase mb-2">Recent Activity</h4>
                  <div className="text-xs font-mono text-gray-400 space-y-1">
                    {messages?.filter(m => m.agent === selectedAgent.name).slice(0, 3).map((m, i) => (
                      <div key={i} className="opacity-70 truncate border-l-2 border-white/10 pl-2">
