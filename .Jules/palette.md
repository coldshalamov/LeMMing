...
**Learning:** When selections are made via a separate modal (like capabilities or tool selection), forcing users to reopen the modal just to remove an item adds unnecessary friction and cognitive load.
**Action:** Always provide inline "remove" buttons on the summary chips displaying the selected items. Ensure these buttons have clear `aria-label`s and visible focus states (`focus-visible:ring-2`) for accessibility.
