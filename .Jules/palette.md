...
## 2025-03-09 - Inline Remove Buttons for Modal Selections
**Learning:** In multi-step forms or wizards with modal selections (like selecting agent capabilities), forcing users to reopen the entire selection modal just to uncheck a single item adds unnecessary friction and cognitive load.
**Action:** Added inline remove buttons (`<button><X size={12} /></button>`) directly on the selected item chips, equipped with proper `aria-label`, `title`, and explicit `focus-visible:ring-2 focus-visible:ring-brand-cyan` styles to ensure single-click removal and keyboard accessibility.
