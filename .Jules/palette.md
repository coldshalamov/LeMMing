...
## 2025-03-08 - Keyboard Focus Indicators on Custom Interactive Elements
**Learning:** Custom interactive elements (like `div`s with `role="button"` or custom modal action buttons) often rely entirely on mouse interactions (`onClick`) and generic styling. If native interactive elements (`button`, `a`) are stripped of their default styling, or if non-native elements are made focusable (`tabIndex={0}`), they must receive explicit visual focus indicators. Failing to do so makes them invisible to keyboard-only users who rely on the `Tab` key to navigate. Tailwind's `focus-visible:` variant combined with `outline-none` and a custom `ring` is perfect for providing clear, branded focus states that only appear during keyboard navigation (not on mouse clicks).
**Action:** Consistently apply `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-[color]` to all custom interactive components or heavily styled native buttons to ensure they remain discoverable and usable via keyboard.
