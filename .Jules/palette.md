...
## 2024-05-24 - Accessibility for Interactive Icon Buttons
**Learning:** For interactive UI elements (like a "Send" button) that dynamically disable based on state, replacing the native `disabled` attribute with `aria-disabled="true"` ensures the element remains focusable via the keyboard tab sequence for screen reader users. To fully replace the native disabled behavior, you must also manually handle `e.preventDefault()` in the `onClick` handler and neutralize hover styles via `aria-disabled:hover:bg-<original>`. Finally, dynamically changing the `aria-label` text can confuse assistive technologies; keep the `aria-label` static (e.g., "Send message") and use the `title` attribute to provide contextual explanations of the disabled state (e.g., "Type a message to send").
**Action:** Applied this pattern to the Manager Chat send button, significantly improving its accessibility and discoverability.
