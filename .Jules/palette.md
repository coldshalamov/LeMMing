...
## 2026-04-03 - Prevent Ghost Tab Stops on Conditionally Expandable Elements
**Learning:** Elements that look like buttons but are only conditionally interactive can confuse screen reader users and keyboard navigators if they always receive focus. Applying `role="button"`, `tabIndex={0}`, and `aria-expanded` attributes statically causes "ghost tab stops".
**Action:** Conditionally set `role`, `tabIndex`, and `aria-*` attributes to `undefined` (not `false` or `0`) when the element is not interactive, and only apply cursor pointer and focus styles when interactive.
