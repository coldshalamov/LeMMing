...
## 2025-03-09 - Inline Removal for Modal Selections
**Learning:** Forcing users to re-open a modal dialog just to deselect an item (like an agent capability) creates unnecessary friction.
**Action:** Implemented inline "remove" buttons on selection chips. These must be semantic `<button>` elements equipped with `aria-label`, `title`, and explicit `focus-visible` styles (`focus-visible:outline-none focus-visible:ring-2`) to guarantee keyboard accessibility within custom components.
