...
## 2025-03-09 - Explain Disabled Button States
**Learning:** Users often struggle to understand why forms won't submit when buttons are disabled without explanation. Native disabled buttons still show a default pointer cursor and no tooltip, leading to "rage clicking".
**Action:** Always add a `title` attribute to disabled buttons explaining *why* they are disabled (e.g. "Enter at least one API key to save") and add `disabled:cursor-not-allowed` for immediate visual feedback before the user even clicks. Also ensure icon-only buttons have descriptive `title` tooltips for hover state discoverability.
