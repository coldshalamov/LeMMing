...
## 2024-05-24 - Dynamic Cursors and aria-disabled
**Learning:** When using `aria-disabled="true"` instead of the native `disabled` attribute, the cursor behavior needs explicit handling for better UX. Use `cursor-wait` for loading/saving states and `cursor-not-allowed` for truly disabled states, combined with `aria-disabled`. Additionally, always manually prevent click handlers when `aria-disabled` is active.
**Action:** Replaced native `disabled` with `aria-disabled` on action buttons in ManagerChat and GlobalSettingsModal, added dynamic tailwind cursor classes (`cursor-wait`/`cursor-not-allowed`) using `clsx`, and added `title` tooltips explaining the disabled state to enhance mouse user UX.
