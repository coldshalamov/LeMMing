...
## 2025-04-05 - Conditional Interaction Attributes in React
**Learning:** Applying `role="button"`, `tabIndex={0}`, and `aria-expanded` globally to an element that is only sometimes interactive creates false expectations for screen reader and keyboard users.
**Action:** When conditionally rendering interactivity (like expand/collapse), use `{canExpand ? "button" : undefined}` (and similarly for `tabIndex` and `aria-*` attributes) to completely strip the attributes when the element isn't actionable, rather than setting them to `false` or leaving them universally applied.
