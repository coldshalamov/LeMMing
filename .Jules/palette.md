...
## 2025-02-13 - Add Inline Remove Buttons to Capability Chips
**Learning:** For multi-step forms or wizards with modal selections, providing inline remove buttons on the selected item chips allows single-click removal without forcing users to re-open the selection modal, which is a common source of friction.
**Action:** Always provide inline remove buttons on selected item chips (equipped with `aria-label` and `focus-visible` styles) to allow single-click removal.
