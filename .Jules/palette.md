...
## 2025-03-31 - Inline Removal for Modal Selections
**Learning:** For multi-step forms or wizards with modal selections, forcing users to re-open the selection modal just to remove an item is a frustrating UX bottleneck. Users expect to be able to quickly adjust their selections directly from the summary view.
**Action:** Provide inline remove buttons (with proper ARIA labels and focus-visible styles) on the selected item chips to allow single-click removal without opening the modal.
