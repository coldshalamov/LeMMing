...
## 2026-04-06 - Native Disabled Attributes Prevent Keyboard Focus\n**Learning:** Native `disabled` attributes remove elements from the tab order, hiding them from screen readers and preventing keyboard navigation.\n**Action:** Use `aria-disabled="true"` combined with custom CSS (`aria-disabled:opacity-50 aria-disabled:cursor-not-allowed`) and a manual JS event guard to ensure disabled UI elements remain discoverable via keyboard while preventing interaction.
